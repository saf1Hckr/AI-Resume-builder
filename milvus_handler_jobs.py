...
client = MilvusClient("Job_Search.db")
...
def create_milvus_collection2(User_id, dimension=1536):
...
def insert_into_milvus2(data, User_Id):
...
def query_milvus2(query_embedding, User_id, limit=5):
    collection_name = get_collection_name(User_id)
    ...
    if not client.has_collection(collection_name=collection_name):
        create_milvus_collection2(User_id, dimension=1536)
    ...
    try:
        ...
        return client.search(
            ...
            limit=limit,
            output_fields=["title","text", "subject", "distance"]
        )
